...
#define _min_interval  	10
#define	_max_interval	15
...
{
  2, AD_TYPE_TX_POWER_LEVEL, 3 /* 3dBm */, /* Transmission Power */
  10, AD_TYPE_COMPLETE_LOCAL_NAME, 'N', 'e', 'u', 'r', 'a', 'n', 'i', 'c', 's',  /* Complete name */
...
//#if (L2CAP_REQUEST_NEW_CONN_PARAM != 0)
//	#define SIZE_TAB_CONN_INT            2
//	a_ConnInterval[0] = 10; /* ms */
//	a_ConnInterval[1] = 25; /* ms */
//#endif /* L2CAP_REQUEST_NEW_CONN_PARAM != 0 */
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      if (p_disconnection_complete_event->Connection_Handle == BleApplicationContext.BleApplicationContext_legacy.connectionHandle)
      {
		...
		#ifdef BLE_Debug
			DIAG (" BLE Debug ->\tDisconnection Complete\n");
			DIAG (" \t\tConnection Handle:\t0x%x\n\t\tReason:\t0x%x\n\r",
                    p_disconnection_complete_event->Connection_Handle,
                    p_disconnection_complete_event->Reason);
		#endif

			// Re-initialise BLE again
			APP_BLE_Init();
...
//          Connection_Interval_Update_Req();
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          tsBle.Connected = true;
			...
			#ifdef BLE_Debug
          	  	  DIAG (" BLE Debug ->\tConnection handle:\t0x%x\n", p_connection_complete_event->Connection_Handle);
				  DIAG (" \t\tBLE Connected\t[ Success: :%02x:%02x:%02x:%02x:%02x:%02x ]\n",
	                      p_connection_complete_event->Peer_Address[5],
	                      p_connection_complete_event->Peer_Address[4],
	                      p_connection_complete_event->Peer_Address[3],
	                      p_connection_complete_event->Peer_Address[2],
